...
class project1:
    def __init__(self, N, Omega_m0, Omega_lambda0, h):
        ...
        self.h = h
        self.Omega_k0 = 1 -  Omega_m0 - Omega_lambda0
        self.c = 299792458    

    def integrate(self, x):
        """ Integral that solves for t*H_0"""
        return np.sqrt(x)/np.sqrt(self.Omega_m0 + self.Omega_k0*x + self.Omega_lambda0*x**3)
    ...
    def integrate_time_z(self, z, Omega_m0, Omega_lambda0, Omega_k0):
        ...
        p = 1.0 + z
        return 1.0/(p*np.sqrt(Omega_m0*p**3 + Omega_k0*p**2 + Omega_lambda0))
    ...
    def run(self):        
        ...
        for i in range(self.N-1, 0, -1):
            x[i-1] = x[i] - H_0*t_0*dt*np.sqrt(self.Omega_m0/x[i] + self.Omega_k0 + self.Omega_lambda0*x[i]**2)    
        plt.plot(t, x)
        ...
        for i in range(self.N):
           time_z[i] = quad(self.integrate_time_z, 0, z[i], args = (self.Omega_m0, self.Omega_lambda0, self.Omega_k0))[0]/H_0
        ...
        for i in range(self.N):
            if self.Omega_k0 == 0:
                dL[i] = (1 + z[i])*quad(self.integrate_z, 0, z[i], args = (self.Omega_m0, self.Omega_lambda0, self.Omega_k0))[0]
            else:
                dL[i]= (1 + z[i])/np.sqrt(abs(self.Omega_k0))*self.S(np.sqrt(abs(self.Omega_k0))*quad(self.integrate_z, 0, z[i], args = (self.Omega_m0, self.Omega_lambda0, self.Omega_k0))[0], self.Omega_k0)
        plt.plot(z, dL)
        plt.xlabel("Z")
        plt.ylabel("dL*c/H_0")
        plt.show()
